...
3. **No load balancing loss:** Unlike earlier MoE work (e.g., GShard, Switch Transformer), Mixtral does **not** use an auxiliary load-balancing loss during training. The router learns to distribute tokens across experts without explicit balancing pressure
...
| **Load Balancing Loss** | An auxiliary loss used in some MoE architectures (e.g., GShard, Switch Transformer) to encourage the router to distribute tokens evenly across experts. Mixtral notably does **not** use this — the router learns balanced routing without explicit pressure. |
